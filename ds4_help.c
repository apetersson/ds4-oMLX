...
static void print_steering(FILE *fp, const help_colors *c, ds4_help_tool tool) {
    ...
    opt(fp, c, "--dir-steering-file FILE", (tool == DS4_HELP_DS4 || tool == DS4_HELP_SERVER) ?
        "Load legacy f32 vectors or a model-bound V4.1 DS41DIR file." :
        "Load one f32 direction vector per layer.");
    if ((tool == DS4_HELP_DS4 || tool == DS4_HELP_SERVER))
        opt(fp, c, "--dir-steering-strength F", "V4.1 only: required finite alpha [-100,100]; site from DS41DIR. 0 disables edits.");
    ...
    if (tool == DS4_HELP_SERVER)
        para(fp, c, "V4.1 steering applies to all slots. Live KV reuse is supported; --kv-disk-dir is incompatible.");
    fputc('\n', fp);
}
